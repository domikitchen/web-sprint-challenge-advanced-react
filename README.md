...
{
  Stateful class components are easily identified by the way they use state in a constructor and their use of render
  typical set up: 
      ...
      class NameOfComponent extends Component {
        constructor() {
          super();
          this.state = {
            nameOfAVariable: "String"
          }
        }

        nameOfFunction = () => {
          this.setState({
            nameOfAVariable: "New String"
          });
        }
...
{
  -Mounting
    -constructor funciton called and state is initalized
  -Updating
    -componentDidUpdate() will set new states. Any props updated will update the components affected
  -Unmounting
}
...
{
  -componentDidMount()
    -while the state is initalized in the constructor, component did mount sets specific state's values often used as the inital values often accessed through an api
  -componentDidUpdate()
    -the state will be changed often after a form submition to recieve and/or send new/different data
  -componentWillUnmount()
    -when the compnent is finished being used it will be removed or "unmounted" from the DOM
}

4. Define stateful logic.
{
  -logic build into a component that uses state
}

5. Describe how to test a React component with React Testing Library.
{
  -You will need to grab any thing from your DOM that you would like to test and then test data that you're expecting to receive to check your components functionality in their intended purpose
}
...
- [x] Set the data to a state property called `this.state.plants` //the data was not in the specified location, though it was found in 'res.data.plantsData'
